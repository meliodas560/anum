...
regex_upvote = r"^((?i)\+|\+\+|\+1|thx|thanx|thanks|pro|cool|good|👍|sip|ok|terimakasih|ty)$"
regex_downvote = r"^(\-|\-\-|\-1|👎|noob|weak|boo|hoax|tipu|scam|bohong)$"
...
async def upvote(_, message):
    ...
    await message.reply_text(
        f"Good Karma of {user_mention} By 1 \nTotal Points: {karma}"
    )
...
async def downvote(_, message):
    ...
    await update_karma(chat_id, await int_to_alpha(user_id), new_karma)
    await message.reply_text(
        f"Bad Karma Of {user_mention} By 1 \nTotal Points: {karma}"
    )
