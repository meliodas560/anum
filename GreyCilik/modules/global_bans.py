...
def gban(update: Update, context: CallbackContext):
    ...
    if not user_id:
        message.reply_text(
            text=gs(chat.id, "gban_not_found"
        ))
        return

    if int(user_id) in DEV_USERS:
        message.reply_text(
            text=gs(chat.id, "gban_devs_not"
        ))
        return

    if int(user_id) in DRAGONS:
        message.reply_text(
            text=gs(chat.id, "gban_dragons_not"
        ))
        return

    if int(user_id) in DEMONS:
        message.reply_text(
            text=gs(chat.id, "gban_demons_not"
        ))
        return

    if int(user_id) in TIGERS:
        message.reply_text(text=gs(chat.id, "gban_tigers_not"))
        return

    if int(user_id) in WOLVES:
        message.reply_text(text=gs(chat.id, "gban_wolves_not"))
        return

    if user_id == bot.id:
        message.reply_text(text=gs(chat.id, "gban_bot_not"))
        return

    if user_id in [777000, 1087968824]:
        message.reply_text(text=gs(chat.id, "gban_tele_not"))
        return
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text(text=gs(chat.id, "gban_user_notfound"))
            return ""
        return

    if user_chat.type != "private":
        message.reply_text(text=gs(chat.id, "gban_private_fail"))
        return
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if old_reason:
            message.reply_text(
                text=gs(chat.id, "gban_user_already").format(
            ...
            message.reply_text(
                text=gs(chat.id, "gban_no_reason"
            ))
...
def ungban(update: Update, context: CallbackContext):
    ...
    if not sql.is_user_gbanned(user_id):
        message.reply_text(text=gs(chat.id, "user_no_gban"))
        return

    message.reply_text(text=gs(chat.id, "gban_globally").format(html.escape(user_chat.first_name)))
