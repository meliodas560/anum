    ...
    if msg.reply_to_message:
        if not msg.reply_to_message.sticker:
            return msg.reply_text(
                text=gs(update.effective_chat.id, "set_sticker"
            ))
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        text=gs(chat.id, "promote_success").format(
...
def fullpromote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        text=gs(chat.id, "full_promote_success").format(
            html.escape(chat.title),
            mention_html(user_member.user.id, user_member.user.first_name),
            mention_html(user.id, user.first_name)   
            ),
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            text=gs(chat.id, "demote_success").format(
                html.escape(chat.title),
                mention_html(user_member.user.id, user_member.user.first_name),
                mention_html(user.id, user.first_name)   
            ),
        ...
        message.reply_text(
            text=gs(update.effective_chat.id, "demote_error"
        ))
        return
...
def set_title(update: Update, context: CallbackContext):
    ...
    if user_member.status == "creator":
        message.reply_text(
             text=gs(update.effective_chat.id, "set_title_creator"
        ))
        return

    if user_member.status != "administrator":
        message.reply_text(
            text=gs(update.effective_chat.id, "set_title_admin"
        ))
        return

    if user_id == bot.id:
        message.reply_text(
            text=gs(update.effective_chat.id, "set_title_self"
        ))
        return
    ...
    if len(title) > 16:
        message.reply_text(
            text=gs(update.effective_chat.id, "set_title_len"
        ))
    ...
    try:
        ...
        message.reply_text(
            text=gs(update.effective_chat.id, "set_title_error"
        ))
        return
...
def pin(update: Update, context: CallbackContext) -> str:
    ...
    if prev_message and is_group:
        try:
            ...
            msg.reply_text(text=gs(chat.id,
                "pin_success"
            ))               
        except BadRequest as excp:
...
def unpin(update: Update, context: CallbackContext):
    ...
    if (
        ...
        message.reply_text(teks=gs(update.effective_chat.id, "unpin_creator"))
        return
    ...
    if not prev_message and is_group:
        try:
            ...
            msg.reply_text(
                teks=gs(update.effective_chat.id, "unpin_last_msg"
            ))
        except BadRequest as excp:
            if excp.message == "Message to unpin not found":
               msg.reply_text(
                   teks=gs(update.effective_chat.id, "unpin_error"
               ))
            else:
                raise
    ...
    if chat.pinned_message:
        ...
        msg.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text=gs(update.effective_chat.id, "pin_button"), 
        ...
        msg.reply_text(
            text=gs(update.effective_chat.id, "pinned_none").format(html.escape(chat.title)),
            parse_mode=ParseMode.HTML,
        )
    ...
    if chat.username:
        ...
        if bot_member.can_invite_users:
            ...
            update.effective_message.reply_text(
                text=gs(update.effective_chat.id, "invite_error1"
            ))
    else:
        update.effective_message.reply_text(
            text=gs(update.effective_chat.id, "invite_error2"
        ))
...
def adminlist(update, context):
    ...
    try:
        msg = update.effective_message.reply_text(
            text=gs(update.effective_chat.id, "list_admin_loading"),
            parse_mode=ParseMode.HTML,
        )
    except BadRequest:
        msg = update.effective_message.reply_text(
...
def button(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        update.effective_message.edit_text(
             text=gs(update.effective_chat.id, "demote_callback_error"
        ))
        return ""
...
FULLPROMOTE_HANDLER = DisableAbleCommandHandler("fulladmin", fullpromote, run_async=True)
