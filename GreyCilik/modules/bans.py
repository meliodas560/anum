...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text(text=gs(chat.id, "bans_notfound"))
        return log_message
    try:
        ...
        if excp.message != "User not found":
            raise
        message.reply_text(text=gs(chat.id, "bans_usernotfound"))
        return log_message
    if user_id == bot.id:
        message.reply_text(text=gs(chat.id, "bans_me"))
        return log_message

    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(text=gs(chat.id, "bans_owner"))
        elif user_id in DEV_USERS:
            message.reply_text(text=gs(chat.id, "bans_devs"))
        elif user_id in DRAGONS:
            message.reply_text(
                text=gs(chat.id, "bans_dragons"
            ))
        elif user_id in DEMONS:
            message.reply_text(
                text=gs(chat.id, "bans_demons"
            ))
        elif user_id in TIGERS:
            message.reply_text(text=gs(chat.id, "bans_tigers"))
        elif user_id in WOLVES:
            message.reply_text(text=gs(chat.id, "bans_wolves"))
        else:
            message.reply_text(text=gs(chat.id, "bans_not_admins"))
        return log_message
    ...
    try:
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text=gs(chat.id, "delete_button"), callback_data="unbanb_del"
                        ),
                    ]
                ]
            ),
        ...
        if excp.message == "Reply message not found":
            ...
            message.reply_text(text=gs(chat.id, "bans_dont_work"))
...
def temp_ban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        if excp.message != "User not found":
            raise
        message.reply_text(text=gs(chat.id, "bans_notuserfound"))
        return log_message
    if user_id == bot.id:
        message.reply_text(text=gs(chat.id, "bans_crazy"))
        return log_message

    if is_user_ban_protected(chat, user_id, member):
        message.reply_text(text=gs(chat.id, "bans_feel"))
        return log_message

    if not reason:
        message.reply_text(text=gs(chat.id, "bans_istime"))
        return log_message
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            message.reply_text(text=gs(chat.id, "bans_fail"))
...
def unbanb_btn(update: Update, context: CallbackContext) -> str:
    ...
    if query.data != "unbanb_del":
        ...
        if not is_user_admin(chat, int(user.id)):
            bot.answer_callback_query(
                query.id,
                text=gs(chat.id, "delete_fail"),
                show_alert=True,
            )
            return ""
        query.message.delete()
        bot.answer_callback_query(query.id, text=gs(chat.id, "delete_done"))
        return ""
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        return log_message
    if user_id == bot.id:
        message.reply_text(text=gs(chat.id, "bans_not_ban"))
        return log_message

    if is_user_ban_protected(chat, user_id):
        message.reply_text(text=gs(chat.id, "bans_user_punch"))
        return log_message
    ...
    if res:
        ...
        message.reply_text(text=gs(chat.id, "bans_cant_punch"))
...
def unban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        return log_message
    if user_id == bot.id:
        message.reply_text(text=gs(chat.id, "bans_fail_botdonthere"))
        return log_message
...
def selfunban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        message.reply_text(text=gs(chat.id, "bans_id_valid"))
        return
    ...
    if is_user_in_chat(chat, user.id):
        message.reply_text(text=gs(chat.id, "bans_ready_chat"))
        return
...
def banme(update: Update, context: CallbackContext):
    ...
    user = update.effective_user
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text(text=gs(chat.id, "bans_not_admins"))
        return

    res = update.effective_chat.ban_member(user_id)
    if res:
        update.effective_message.reply_text(text=gs(chat.id, "bans_gtfo"))
        return (
            "<b>{}:</b>"
            "\n#BANME"
            "\n<b>User:</b> {}"
        ...
        update.effective_message.reply_text(text=gs(chat.id, "bans_i_cant"))
...
def snipe(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.effective_message.reply_text(text=gs(chat.id, "bans_snipe_echo"))
    to_send = " ".join(args)
    if len(to_send) >= 2:
        try:
            ...
            update.effective_message.reply_text(
                text=gs(chat.id, "bans_notpart_snipe"
            ))
